import { HttpResponse, http } from "msw";
import {
  InMemoryStorageAdapter,
  IntentManager,
  IntentSynchronizer,
  SandshrewRpcProvider,
} from "../src";
import { IntentStatus, IntentType } from "../src/types";
import { server } from "./mocks/server";
import { WITNESS_SCRIPTS } from "./mocks/constants";

beforeAll(() => server.listen());
afterEach(() => server.resetHandlers());
afterAll(() => server.close());

type SandshrewRpcProviderRequestBody = {
  method: string;
  params: string[];
};

test("Updates intent as completed for confirmed transactions", async () => {
  server.use(
    http.post("http://localhost:3000/v1/regtest", async ({ request }) => {
      const { method } = (await request
        .clone()
        .json()) as SandshrewRpcProviderRequestBody;

      if (method === "esplora_tx") {
        return HttpResponse.json({
          result: {
            status: {
              confirmed: true,
            },
          },
        });
      }
    })
  );

  const storage = new InMemoryStorageAdapter();
  const intentManager = new IntentManager(storage);
  const intentSyncronizer = new IntentSynchronizer(
    intentManager,
    new SandshrewRpcProvider({
      network: "regtest",
      projectId: "123",
    })
  );

  await intentManager.captureIntent({
    address: "tb1p6qyjjf9037p3sshkmaum2ylgzwx353ts05zmrtvagu4wva6psrgqv0w7ln",
    type: IntentType.Transaction,
    status: IntentStatus.Pending,
    data: { txIds: ["confirmedTx"] },
  });

  let intents = await storage.getAllIntents();
  expect(intents[0]).toHaveProperty("status", "pending");

  await intentSyncronizer.syncIntents();

  intents = await storage.getAllIntents();
  expect(intents[0]).toHaveProperty("status", "completed");
});

test("Receive BTC confirmed", async () => {
  server.use(
    http.post("http://localhost:3000/v1/regtest", async ({ request }) => {
      const { method } = (await request
        .clone()
        .json()) as SandshrewRpcProviderRequestBody;

      if (method === "esplora_address::txs") {
        return HttpResponse.json({
          result: [
            {
              id: "1bd07c9c92c56ff1d74a45e3b72fb7c0a5de02ca51bed4741b1c4c74f166e88f",
              vin: [],
              vout: [
                {
                  scriptpubkey_address:
                    "tb1pdykkv4ldhmw2n9mpehffjk7dszltheqkhjtg3hj7p97u33jja8cq4fuph7",
                },
              ],
              status: {
                confirmed: true,
              },
            },
          ],
        });
      }

      if (method === "ord_output") {
        return HttpResponse.json({
          result: {
            indexed: true,
            inscriptions: [],
            runes: [],
          },
        });
      }
    })
  );

  const storage = new InMemoryStorageAdapter();
  const intentManager = new IntentManager(storage);
  const intentSyncronizer = new IntentSynchronizer(
    intentManager,
    new SandshrewRpcProvider({
      network: "regtest",
      projectId: "123",
    })
  );

  let intents = await storage.getAllIntents();
  expect(intents).toHaveLength(0);

  await intentSyncronizer.syncReceivedTxIntents([
    "tb1pdykkv4ldhmw2n9mpehffjk7dszltheqkhjtg3hj7p97u33jja8cq4fuph7",
  ]);

  intents = await storage.getAllIntents();
  expect(intents).toHaveLength(1);
  expect(intents[0]).toHaveProperty("type", "transaction");
  expect(intents[0]).toHaveProperty("status", "completed");
  expect(intents[0]).toHaveProperty("data.txIds", [
    "1bd07c9c92c56ff1d74a45e3b72fb7c0a5de02ca51bed4741b1c4c74f166e88f",
  ]);
  expect(intents[0]).toHaveProperty("data.collectibles", []);
  expect(intents[0]).toHaveProperty("data.brc20s", []);
  expect(intents[0]).toHaveProperty("data.runes", []);
});

test("Receive BTC unconfirmed", async () => {
  server.use(
    http.post("http://localhost:3000/v1/regtest", async ({ request }) => {
      const { method } = (await request
        .clone()
        .json()) as SandshrewRpcProviderRequestBody;

      if (method === "esplora_address::txs") {
        return HttpResponse.json({
          result: [
            {
              id: "1bd07c9c92c56ff1d74a45e3b72fb7c0a5de02ca51bed4741b1c4c74f166e88f",
              vin: [],
              vout: [
                {
                  scriptpubkey_address:
                    "tb1pdykkv4ldhmw2n9mpehffjk7dszltheqkhjtg3hj7p97u33jja8cq4fuph7",
                },
              ],
              status: {
                confirmed: false,
              },
            },
          ],
        });
      }
    })
  );

  const storage = new InMemoryStorageAdapter();
  const intentManager = new IntentManager(storage);
  const intentSyncronizer = new IntentSynchronizer(
    intentManager,
    new SandshrewRpcProvider({
      network: "regtest",
      projectId: "123",
    })
  );

  let intents = await storage.getAllIntents();
  expect(intents).toHaveLength(0);

  await intentSyncronizer.syncReceivedTxIntents([
    "tb1pdykkv4ldhmw2n9mpehffjk7dszltheqkhjtg3hj7p97u33jja8cq4fuph7",
  ]);

  intents = await storage.getAllIntents();
  expect(intents).toHaveLength(1);
  expect(intents[0]).toHaveProperty("type", "transaction");
  expect(intents[0]).toHaveProperty("status", "pending");
  expect(intents[0]).toHaveProperty("data.txIds", [
    "1bd07c9c92c56ff1d74a45e3b72fb7c0a5de02ca51bed4741b1c4c74f166e88f",
  ]);
  expect(intents[0]).toHaveProperty("data.collectibles", []);
  expect(intents[0]).toHaveProperty("data.brc20s", []);
  expect(intents[0]).toHaveProperty("data.runes", []);
});

test("Confirmed TX with Collectible in Outputs", async () => {
  server.use(
    http.post("http://localhost:3000/v1/regtest", async ({ request }) => {
      const { method } = (await request
        .clone()
        .json()) as SandshrewRpcProviderRequestBody;

      if (method === "esplora_address::txs") {
        return HttpResponse.json({
          result: [
            {
              id: "1bd07c9c92c56ff1d74a45e3b72fb7c0a5de02ca51bed4741b1c4c74f166e88f",
              vin: [],
              vout: [
                {
                  scriptpubkey_address:
                    "tb1pdykkv4ldhmw2n9mpehffjk7dszltheqkhjtg3hj7p97u33jja8cq4fuph7",
                },
              ],
              status: {
                confirmed: true,
              },
            },
          ],
        });
      }

      if (method === "ord_output") {
        return HttpResponse.json({
          result: {
            indexed: true,
            inscriptions: [
              "bb9ca79081bc51d968ab1b41766ccf4a5e920161e42fa1cb8854c853a83cc0cei0",
            ],
            runes: [],
          },
        });
      }

      if (method === "ord_inscription") {
        return HttpResponse.json({
          result: {
            id: "bb9ca79081bc51d968ab1b41766ccf4a5e920161e42fa1cb8854c853a83cc0cei0",
            content_type: "image/png",
          },
        });
      }
    })
  );

  const storage = new InMemoryStorageAdapter();
  const intentManager = new IntentManager(storage);
  const intentSyncronizer = new IntentSynchronizer(
    intentManager,
    new SandshrewRpcProvider({
      network: "regtest",
      projectId: "123",
    })
  );

  await intentSyncronizer.syncReceivedTxIntents([
    "tb1pdykkv4ldhmw2n9mpehffjk7dszltheqkhjtg3hj7p97u33jja8cq4fuph7",
  ]);

  const intents = await storage.getAllIntents();

  expect(intents).toHaveLength(1);
  expect(intents[0]).toHaveProperty("type", "transaction");
  expect(intents[0]).toHaveProperty("status", "completed");
  expect(intents[0].data).toHaveProperty("txIds", [
    "1bd07c9c92c56ff1d74a45e3b72fb7c0a5de02ca51bed4741b1c4c74f166e88f",
  ]);
  expect(intents[0].data).toHaveProperty(
    "collectibles[0].id",
    "bb9ca79081bc51d968ab1b41766ccf4a5e920161e42fa1cb8854c853a83cc0cei0"
  );
  expect(intents[0].data).toHaveProperty(
    "collectibles[0].content_type",
    "image/png"
  );
  expect(intents[0].data).toHaveProperty("brc20s", []);
  expect(intents[0].data).toHaveProperty("runes", []);
});

test("Unconfirmed TX with Collectible in Prevout", async () => {
  server.use(
    http.post("http://localhost:3000/v1/regtest", async ({ request }) => {
      const { method, params } = (await request
        .clone()
        .json()) as SandshrewRpcProviderRequestBody;

      if (method === "esplora_address::txs") {
        return HttpResponse.json({
          result: [
            {
              id: "1bd07c9c92c56ff1d74a45e3b72fb7c0a5de02ca51bed4741b1c4c74f166e88f",
              vin: [
                {
                  txid: "68bf2613e71cf8cc8652bba6f138d713cf44992eb067b8eb35b707e9a35c4105",
                  vout: 1,
                  prevout: {},
                  witness: [],
                },
              ],
              vout: [
                {
                  scriptpubkey_address:
                    "tb1pdykkv4ldhmw2n9mpehffjk7dszltheqkhjtg3hj7p97u33jja8cq4fuph7",
                },
              ],
              status: {
                confirmed: false,
              },
            },
          ],
        });
      }

      if (method === "ord_output") {
        // TX output not indexed
        if (
          params[0] ===
          "1bd07c9c92c56ff1d74a45e3b72fb7c0a5de02ca51bed4741b1c4c74f166e88f:0"
        ) {
          return HttpResponse.json({
            result: {
              indexed: false,
              inscriptions: [],
              runes: [],
            },
          });
        }

        // TX prevout indexed with inscriptions
        if (
          params[0] ===
          "68bf2613e71cf8cc8652bba6f138d713cf44992eb067b8eb35b707e9a35c4105:1"
        ) {
          return HttpResponse.json({
            result: {
              indexed: true,
              inscriptions: [
                "bb9ca79081bc51d968ab1b41766ccf4a5e920161e42fa1cb8854c853a83cc0cei0",
              ],
              runes: [],
            },
          });
        }
      }

      if (method === "ord_inscription") {
        return HttpResponse.json({
          result: {
            id: "bb9ca79081bc51d968ab1b41766ccf4a5e920161e42fa1cb8854c853a83cc0cei0",
            content_type: "image/png",
          },
        });
      }
    })
  );

  const storage = new InMemoryStorageAdapter();
  const intentManager = new IntentManager(storage);
  const intentSyncronizer = new IntentSynchronizer(
    intentManager,
    new SandshrewRpcProvider({
      network: "regtest",
      projectId: "123",
    })
  );

  await intentSyncronizer.syncReceivedTxIntents([
    "tb1pdykkv4ldhmw2n9mpehffjk7dszltheqkhjtg3hj7p97u33jja8cq4fuph7",
  ]);

  const intents = await storage.getAllIntents();

  expect(intents).toHaveLength(1);
  expect(intents[0]).toHaveProperty("type", "transaction");
  expect(intents[0]).toHaveProperty("status", "pending");
  expect(intents[0].data).toHaveProperty("txIds", [
    "1bd07c9c92c56ff1d74a45e3b72fb7c0a5de02ca51bed4741b1c4c74f166e88f",
  ]);
  expect(intents[0].data).toHaveProperty(
    "collectibles[0].id",
    "bb9ca79081bc51d968ab1b41766ccf4a5e920161e42fa1cb8854c853a83cc0cei0"
  );
  expect(intents[0].data).toHaveProperty(
    "collectibles[0].content_type",
    "image/png"
  );
  expect(intents[0].data).toHaveProperty("brc20s", []);
  expect(intents[0].data).toHaveProperty("runes", []);
});

test("Uconfirmed TX with Collectible in Input Witness", async () => {
  server.use(
    http.post("http://localhost:3000/v1/regtest", async ({ request }) => {
      const { method, params } = (await request
        .clone()
        .json()) as SandshrewRpcProviderRequestBody;

      if (method === "esplora_address::txs") {
        return HttpResponse.json({
          result: [
            {
              id: "1bd07c9c92c56ff1d74a45e3b72fb7c0a5de02ca51bed4741b1c4c74f166e88f",
              vin: [
                {
                  txid: "68bf2613e71cf8cc8652bba6f138d713cf44992eb067b8eb35b707e9a35c4105",
                  vout: 1,
                  prevout: {},
                  witness: WITNESS_SCRIPTS.IMAGE_PNG,
                },
              ],
              vout: [
                {
                  scriptpubkey_address:
                    "tb1pdykkv4ldhmw2n9mpehffjk7dszltheqkhjtg3hj7p97u33jja8cq4fuph7",
                },
              ],
              status: {
                confirmed: false,
              },
            },
          ],
        });
      }

      if (method === "ord_output") {
        // TX output not indexed
        if (
          params[0] ===
          "1bd07c9c92c56ff1d74a45e3b72fb7c0a5de02ca51bed4741b1c4c74f166e88f:0"
        ) {
          return HttpResponse.json({
            result: {
              indexed: false,
              inscriptions: [],
              runes: [],
            },
          });
        }

        // TX prevout indexed with inscriptions
        if (
          params[0] ===
          "68bf2613e71cf8cc8652bba6f138d713cf44992eb067b8eb35b707e9a35c4105:1"
        ) {
          return HttpResponse.json({
            result: {
              indexed: false,
              inscriptions: [],
              runes: [],
            },
          });
        }
      }

      if (method === "ord_inscription") {
        return HttpResponse.json({
          result: {
            id: "bb9ca79081bc51d968ab1b41766ccf4a5e920161e42fa1cb8854c853a83cc0cei0",
            content_type: "image/png",
          },
        });
      }
    })
  );

  const storage = new InMemoryStorageAdapter();
  const intentManager = new IntentManager(storage);
  const intentSyncronizer = new IntentSynchronizer(
    intentManager,
    new SandshrewRpcProvider({
      network: "regtest",
      projectId: "123",
    })
  );

  await intentSyncronizer.syncReceivedTxIntents([
    "tb1pdykkv4ldhmw2n9mpehffjk7dszltheqkhjtg3hj7p97u33jja8cq4fuph7",
  ]);

  const intents = await storage.getAllIntents();

  expect(intents).toHaveLength(1);
  expect(intents[0]).toHaveProperty("type", "transaction");
  expect(intents[0]).toHaveProperty("status", "pending");
  expect(intents[0].data).toHaveProperty("txIds", [
    "1bd07c9c92c56ff1d74a45e3b72fb7c0a5de02ca51bed4741b1c4c74f166e88f",
  ]);
  expect(intents[0].data).toHaveProperty(
    "collectibles[0].id",
    "68bf2613e71cf8cc8652bba6f138d713cf44992eb067b8eb35b707e9a35c4105i0"
  );
  expect(intents[0].data).toHaveProperty(
    "collectibles[0].content_type",
    "image/png"
  );
  expect(intents[0].data).toHaveProperty(
    "collectibles[0].content",
    ""
  );
  expect(intents[0].data).toHaveProperty("brc20s", []);
  expect(intents[0].data).toHaveProperty("runes", []);
});

test("Uconfirmed TX with BRC-20 in Input Witness", async () => {
  server.use(
    http.post("http://localhost:3000/v1/regtest", async ({ request }) => {
      const { method, params } = (await request
        .clone()
        .json()) as SandshrewRpcProviderRequestBody;

      if (method === "esplora_address::txs") {
        return HttpResponse.json({
          result: [
            {
              id: "1bd07c9c92c56ff1d74a45e3b72fb7c0a5de02ca51bed4741b1c4c74f166e88f",
              vin: [
                {
                  txid: "68bf2613e71cf8cc8652bba6f138d713cf44992eb067b8eb35b707e9a35c4105",
                  vout: 1,
                  prevout: {},
                  witness: WITNESS_SCRIPTS.BRC20_DEPLOY,
                },
              ],
              vout: [
                {
                  scriptpubkey_address:
                    "tb1pdykkv4ldhmw2n9mpehffjk7dszltheqkhjtg3hj7p97u33jja8cq4fuph7",
                },
              ],
              status: {
                confirmed: false,
              },
            },
          ],
        });
      }

      if (method === "ord_output") {
        // TX output not indexed
        if (
          params[0] ===
          "1bd07c9c92c56ff1d74a45e3b72fb7c0a5de02ca51bed4741b1c4c74f166e88f:0"
        ) {
          return HttpResponse.json({
            result: {
              indexed: false,
              inscriptions: [],
              runes: [],
            },
          });
        }

        // TX prevout indexed with inscriptions
        if (
          params[0] ===
          "68bf2613e71cf8cc8652bba6f138d713cf44992eb067b8eb35b707e9a35c4105:1"
        ) {
          return HttpResponse.json({
            result: {
              indexed: false,
              inscriptions: [],
              runes: [],
            },
          });
        }
      }

      if (method === "ord_inscription") {
        return HttpResponse.json({
          result: {
            id: "bb9ca79081bc51d968ab1b41766ccf4a5e920161e42fa1cb8854c853a83cc0cei0",
            content_type: "image/png",
          },
        });
      }
    })
  );

  const storage = new InMemoryStorageAdapter();
  const intentManager = new IntentManager(storage);
  const intentSyncronizer = new IntentSynchronizer(
    intentManager,
    new SandshrewRpcProvider({
      network: "regtest",
      projectId: "123",
    })
  );

  await intentSyncronizer.syncReceivedTxIntents([
    "tb1pdykkv4ldhmw2n9mpehffjk7dszltheqkhjtg3hj7p97u33jja8cq4fuph7",
  ]);

  const intents = await storage.getAllIntents();

  expect(intents).toHaveLength(1);
  expect(intents[0]).toHaveProperty("type", "transaction");
  expect(intents[0]).toHaveProperty("status", "pending");
  expect(intents[0].data).toHaveProperty("txIds", [
    "1bd07c9c92c56ff1d74a45e3b72fb7c0a5de02ca51bed4741b1c4c74f166e88f",
  ]);
  expect(intents[0]).toHaveProperty("data.collectibles", []);
  expect(intents[0].data).toHaveProperty("brc20s[0].p", "brc-20");
  expect(intents[0].data).toHaveProperty("brc20s[0].op", "deploy");
  expect(intents[0].data).toHaveProperty("brc20s[0].tick", "PUPS");
  expect(intents[0].data).toHaveProperty("brc20s[0].max", "7770000");
  expect(intents[0].data).toHaveProperty("brc20s[0].lim", "777");
  expect(intents[0].data).toHaveProperty("runes", []);
});
